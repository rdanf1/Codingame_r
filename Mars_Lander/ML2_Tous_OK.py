...
while True:
    ...
    if x < (land_zone_x_begin - (hs * 40)) :
        direction = "right"
    elif x > (land_zone_x_end - (hs * 40)) :
        direction = "left"
    else :
        direction = "straight"
    ...
    if abs(hs) >= 20 :
        ...
        if not horizontal_speed_positive : print(-25, speed )
        else : print(25, speed )

    else :

        if (land_zone_x_begin - (hs * 18) ) < x < (land_zone_x_end - (hs * 18))  :
